...
(async()=>{
 ...
 const header=post.slice(post.indexOf('<header>'),post.indexOf('</header>'));
 assert.ok(header.includes('class="site-nav"') && header.includes('<summary') && header.includes('aria-label="主导航"'),'collapsible site navigation');
 assert.ok(header.includes('class="theme"') && !/<nav[\s\S]*class="theme"[\s\S]*<\/nav>/.test(header),'theme control stays outside navigation');
 ...
 assert.ok(homepage.includes('展开内容') && homepage.includes('<table>') && homepage.includes('highlight'),'homepage HTML, table and code support');
 ...
 await hexo.exit();
 console.log(JSON.stringify({passed:true,hexo:require(req.resolve('hexo/package.json')).version,output:path.join(base,'public'),checks:36}));
